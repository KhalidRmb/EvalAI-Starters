...
if __name__ == "__main__":
	...
	if not is_valid:
		if VALIDATION_STEP=="True" and check_if_pull_request():
			...
			add_pull_request_comment(GITHUB_AUTH_TOKEN, os.path.basename(GITHUB_REPOSITORY), pr_number, errors)
			print("\nExiting the {} script after failure\n".format(os.path.basename(__file__)))
			sys.exit(1)
		else:
			issue_title = "Following errors occurred while validating the challenge config:"
			create_github_repository_issue(GITHUB_AUTH_TOKEN, os.path.basename(GITHUB_REPOSITORY), issue_title, errors)
			print("\nExiting the {} script after failure\n".format(os.path.basename(__file__)))
			sys.exit(1)
